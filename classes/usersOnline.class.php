...
CREATE TABLE `usersonline`(     `id` INT(10) NOT NULL AUTO_INCREMENT ,     `ip` VARCHAR(15) NOT NULL ,     `timestamp` VARCHAR(15) NOT NULL ,     PRIMARY KEY (`id`)  );
*/
...
class usersOnline {
	...
	*/
	...
	function new_user($uname) {
	$uname = mysqli_real_escape_string($dbh,$uname);
		$insert = mysqli_query ($dbh,"INSERT INTO usersonline(timestamp, ip, username) VALUES ('$this->timestamp', '$this->ip', '$uname')");
		if (!$insert) {
	...
	function delete_user() {
		$delete = mysqli_query ("DELETE FROM usersonline WHERE timestamp < ($this->timestamp - $this->timeout)");
		if (!$delete) {
	...
	function count_users() {
		if (count($this->error) == 0) {
			$count = mysql_num_rows ( mysqli_query("SELECT DISTINCT ip FROM usersonline"));
			return $count;
		}
	}
